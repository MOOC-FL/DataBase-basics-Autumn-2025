...
Error: CHECK constraint failed: Products
```
- When we try to insert Productsa row into the table where the price is negative, this violates the condition price >= 0and SQLite does not allow the row to be inserted but gives an error CHECK constraint failed: Products. The same thing happens if we try to change the price column of an existing row to negative afterwards.
